...
let model_check formula lts arguments environment v_lvl indent =
  let rec model_check' form lts args env v_lvl ind =
    match form with
      F.Const(b) -> if b then 
                      ...
                      (S.empty_base lts,env)

    | F.Prop(prop) -> (S.Base(Bddlts.get_prop lts prop),env)

    | F.Var(var,var_t,fp_t) ->  let current_val = 
    ...
    | F.Neg(phi) -> let (bdd,env) = model_check' phi lts args env v_lvl ind in
                    (
                      match bdd with 
                        S.Base bdd -> (S.Base(MLBDD.dnot bdd), env) 
                      | _ -> assert false
                    )
  ...
  | F.Mu(var,var_t,phi) ->  let fp_var = F.Var(var,var_t,LFP) in
  ...
  | F.Nu(var,var_t,phi) ->  let fp_var = F.Var(var,var_t,GFP) in
                            let rec repeat_until map =
                              (*repeat until approximation is found (i.e. fixpoint) 
                                returns env including fp approximation for fp_var.
                              *)
  ...
  | F.Lambda(var,var_t,phi) ->  model_check' 
                                  phi lts 
                                  (List.tl args) 
  ...
  in
  let (bdd,_) = 
        model_check' formula lts arguments environment v_lvl indent in
  match bdd with 
    Base bdd -> Bddlts.get_all_sats lts bdd 
  | _ -> assert false
