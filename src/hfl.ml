...
module Semantics = struct
  ...
  let rec compare = function
    Base base_one ->  (function
                      Base base_two ->  if MLBDD.equal base_one base_two then 0 
                                      else Int.compare (MLBDD.id base_one) (MLBDD.id base_two)
                    | Fun map_two -> assert false)

  | Fun map_one ->  (function
                      Base base_two -> assert false
  ...
  let rec to_string ?(max_length = 200) sem = 
    match sem with
      Base(base) -> MLBDD.to_string base
    | Fun(map) -> let value = 
                    (TreeMap.fold 
  ...
  let full_base lts = Base (Bddlts.get_allstates lts)
                      ...
                      | h :: t -> if is_defined_for_args (Fun map) [h] then
                                    Fun
                                      (
                                        TreeMap.add 
                                          h 
                                          (set_value_for_args value (TreeMap.find h map) t) map
                                      )
                                else
  ...
  let rec from_list_of_pairs = function
    [] -> empty_fun
  | h :: t -> let (arg,value) = h in 
              set_value_for_args value (from_list_of_pairs t) [arg]

  let rec all_of_type lts = function
    (*TODO: Do not use TreeSet -> Speedup with efficient recursion. 
                      ...
                      (fun sub bdd_list -> 
                        Base
                          ...
                          TreeSet.fold
                            ...
                            sub
                            (MLBDD.dfalse (Bddlts.get_man lts))
                          ) 
                        :: bdd_list
                      )
  ...
  | Formula.Fun(arg_t, val_t) ->  let all_arguments = all_of_type lts arg_t in
                                  ...
                                  let rec helper arguments values pairs =
                                    ...
                                    | h :: [] ->  List.fold_left 
                                                    (
                                                      fun val_sems value -> 
                                                        (from_list_of_pairs ((h,value)::pairs)) :: val_sems
                                                    ) 
                                                    [] 
                                                    values
                                    | h :: t -> List.fold_left 
                                                  (
                                                    fun val_sems value -> 
                                                      val_sems @ (helper t values ((h, value) :: pairs))
                                                  ) 
                                                  [] 
                                                  values
                                  in
